-- Migration 102: Remove extension tables created by mistake in 101.
-- recruiter and community_manager are just users with a different role value
-- in the users table. No separate extension table is needed.

DROP TABLE IF EXISTS recruiters_extension;
DROP TABLE IF EXISTS community_managers_extension;

-- Restore get_user_complete() — new roles fall through to ELSE (base data only)
...
BEGIN
  ...
  CASE v_role
    ...
    ELSE
      -- recruiter, community_manager and any future role: base user data is enough
...
-- Restore create_user_with_role() — new roles only need a row in users
...
-- Restore change_user_role() — same logic
