-- Migration 101: Add recruiter and community_manager roles for Enlite internal staff
-- These roles access the admin panel. New @enlite.health Google logins are provisioned
-- as 'recruiter' by default. Admins must be explicitly promoted.

-- Step 1: Update CHECK constraint on users.role
ALTER TABLE users DROP CONSTRAINT IF EXISTS users_role_check;
ALTER TABLE users ADD CONSTRAINT users_role_check
  CHECK (role IN ('worker', 'admin', 'manager', 'client', 'support', 'recruiter', 'community_manager'));

-- Step 2: Create recruiters_extension table
CREATE TABLE IF NOT EXISTS recruiters_extension (
...
-- Step 3: Create community_managers_extension table
CREATE TABLE IF NOT EXISTS community_managers_extension (
  user_id      VARCHAR(128) PRIMARY KEY REFERENCES users(firebase_uid) ON DELETE CASCADE,
  department   VARCHAR(100),
  last_login_at TIMESTAMP WITH TIME ZONE,
  login_count  INTEGER DEFAULT 0,
...
-- Step 4: Update get_user_complete() to handle new roles
...
BEGIN
  ...
  CASE v_role
    ...
    WHEN 'admin' THEN
      SELECT json_build_object('user', v_user_data, 'admin_data', row_to_json(ae.*))
      INTO v_user_data FROM admins_extension ae WHERE ae.user_id = p_firebase_uid;

    WHEN 'recruiter' THEN
      SELECT json_build_object('user', v_user_data, 'recruiter_data', row_to_json(re.*))
      INTO v_user_data FROM recruiters_extension re WHERE re.user_id = p_firebase_uid;

    WHEN 'community_manager' THEN
      SELECT json_build_object('user', v_user_data, 'community_manager_data', row_to_json(cme.*))
      INTO v_user_data FROM community_managers_extension cme WHERE cme.user_id = p_firebase_uid;
...
-- Step 5: Update create_user_with_role() to handle new roles
...
BEGIN
  ...
  CASE p_role
    ...
    WHEN 'recruiter' THEN
      INSERT INTO recruiters_extension (user_id, department)
      VALUES (p_firebase_uid, p_role_data->>'department')
      ON CONFLICT (user_id) DO NOTHING;

    WHEN 'community_manager' THEN
      INSERT INTO community_managers_extension (user_id, department)
      VALUES (p_firebase_uid, p_role_data->>'department')
      ON CONFLICT (user_id) DO NOTHING;
...
-- Step 6: Update change_user_role() to handle new roles
...
BEGIN
  ...
  -- Old extension data is preserved to allow role history / reversal
