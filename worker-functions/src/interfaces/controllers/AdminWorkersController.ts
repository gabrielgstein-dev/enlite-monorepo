 ...
 * - GET /api/admin/workers/:id   - Detalhes completos de um worker
...
export class AdminWorkersController {
   ...
   * Retorna detalhes completos de um worker (dados pessoais, profissionais, documentos, localização, encuadres).
   * Descriptografa campos PII em paralelo via KMS.
   */
  async getWorkerById(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;

      // 1. Buscar worker base
      const workerResult = await this.db.query(
        ...
        FROM workers w
        WHERE w.id = $1 AND w.merged_into_id IS NULL`,
        [id],
      );
      ...
      // 2. Decrypt PII + fetch related data in parallel
      const [
        ...
        this.db.query(
          `SELECT id, address_line, latitude, longitude, radius_km
          FROM worker_service_areas WHERE worker_id = $1`,
          [id],
        ),
        this.db.query(
          `SELECT address, city, work_zone, interest_zone
        ...
        this.db.query(
          `SELECT e.id, e.job_posting_id, jp.case_number,
            ...
            e.rejection_reason, e.rejection_reason_category,
            e.attended, e.created_at
          FROM encuadres e
      ...
      // 3. Parse languages JSON
      let parsedLanguages: string[] = [];
      if (languages) {
        try { parsedLanguages = JSON.parse(languages); } catch { parsedLanguages = [languages]; }
      }

      // 4. Compute eligibility inline (worker_eligibility view was dropped)
      ...
      res.status(200).json({
        ...
        data: {
          ...
          documentType: w.document_type ?? null,
          documentNumber: documentNumber ?? null,
          profilePhotoUrl: profilePhotoUrl ?? null,

          profession: w.profession ?? null,
          occupation: w.occupation ?? null,
          knowledgeLevel: w.knowledge_level ?? null,
          titleCertificate: w.title_certificate ?? null,
          experienceTypes: w.experience_types ?? [],
          yearsExperience: w.years_experience ?? null,
          preferredTypes: w.preferred_types ?? [],
          preferredAgeRange: w.preferred_age_range ?? null,
          ...
          hobbies: w.hobbies ?? [],
          diagnosticPreferences: w.diagnostic_preferences ?? [],
          ...
          documents: doc ? {
            id: doc.id,
            resumeCvUrl: doc.resume_cv_url ?? null,
          ...
          serviceAreas: serviceAreasResult.rows.map((sa: any) => ({
            id: sa.id,
            address: sa.address_line ?? null,
            serviceRadiusKm: sa.radius_km ?? null,
          ...
          location: loc ? {
            ...
            city: loc.city ?? null,
            workZone: loc.work_zone ?? null,
            interestZone: loc.interest_zone ?? null,
          } : null,

          encuadres: encuadresResult.rows.map((e: any) => ({
            id: e.id,
            jobPostingId: e.job_posting_id ?? null,
            caseNumber: e.case_number ?? null,
            patientName: [e.patient_first_name, e.patient_last_name].filter(Boolean).join(' ') || null,
            resultado: e.resultado ?? null,
            interviewDate: e.interview_date ?? null,
            interviewTime: e.interview_time ?? null,
            recruiterName: e.recruiter_name ?? null,
            coordinatorName: e.coordinator_name ?? null,
            rejectionReason: e.rejection_reason ?? null,
            rejectionReasonCategory: e.rejection_reason_category ?? null,
            attended: e.attended ?? null,
