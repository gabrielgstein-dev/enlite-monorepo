 ...
 *
 * Cenários:
 * 1. Enfileira mensagem com 3 opções de horário formatadas
 * 2. Pula envio se vaga não encontrada
 * 3. Pula envio se vaga sem meet links configurados
 * 4. Pula envio se worker não encontrado
 * 5. Tokeniza nome do worker via TokenService
 * 6. Publica outbox-enqueued no Pub/Sub após inserir na outbox
 * 7. Marca interview_response = 'pending' na worker_job_applications
 * 8. Funciona com apenas 1 ou 2 meet links (opções parciais)
 * 9. formatSlotOption formata datetime corretamente
 */
...
describe('QualifiedInterviewHandler', () => {
  ...
  it('enfileira mensagem com 3 opções de horário formatadas', async () => {
    mockQuery
      // 1. SELECT job_posting
    ...
    // Verifica INSERT na outbox com template correto
    const insertCall = mockQuery.mock.calls[2];
    expect(insertCall[0]).toContain('qualified_interview_invite');
    const vars = JSON.parse(insertCall[1][1]);
    expect(vars.name).toBe('tk_abc123def456');
    expect(vars.option_1).toBe('Mar 07/04 10:00');
    expect(vars.option_2).toBe('Mié 08/04 15:00');
    expect(vars.option_3).toBe('Jue 09/04 09:00');
  ...
  it('pula envio se vaga sem meet links configurados', async () => {
    const emptyVacancy = {
      meet_link_1: null,
  ...
  it('tokeniza nome do worker via TokenService', async () => {
    mockQuery
      .mockResolvedValueOnce({ rows: [vacancyRow] })
      .mockResolvedValueOnce({ rows: [{ id: 'worker-1' }] })
      .mockResolvedValueOnce({ rows: [{ id: 'outbox-1' }] })
    ...
    expect(mockTokenService.generate).toHaveBeenCalledWith('worker-1', 'worker_first_name');

    // Token aparece como variável 'name' no outbox
    const insertCall = mockQuery.mock.calls[2];
    const vars = JSON.parse(insertCall[1][1]);
    expect(vars.name).toBe('tk_abc123def456');
  });
