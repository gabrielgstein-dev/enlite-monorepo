...
const STEP_ICONS: Record<WorkerRegistrationStep, (isActive: boolean) => JSX.Element> = {
  'general-info': (isActive) => (
    <div className={`w-[42px] h-[42px] rounded-[10px] flex items-center justify-center flex-shrink-0 relative ${isActive ? 'bg-[#180149]' : 'bg-[#e5e5e5]'}`}>
...
export function Stepper({ className = '' }: StepperProps) {
  ...
  return (
    <div className={`flex items-center justify-between w-full max-w-[1120px] ${className}`}>
      {WORKER_REGISTRATION_STEPS.map((step, index) => {
        ...
        return (
          <div key={step} className="flex items-center gap-[10px] flex-1 last:flex-none" style={{ flex: isLast ? 'none' : '1' }}>
            <button
              type="button"
              onClick={() => isClickable && setCurrentStep(step)}
              disabled={!isClickable}
              className={`
                inline-flex items-center gap-2.5 relative flex-[0_0_auto] bg-transparent outline-none border-none p-0
              ...
              <div className="inline-flex items-center gap-3 relative flex-[0_0_auto]">
                <div
                  className={`
                    ...
                    font-lexend text-[14px] font-medium text-center tracking-[0] leading-[150%] whitespace-nowrap
            ...
            {!isLast && (
              <div className="flex-1 flex justify-center w-full">
                <svg width="11" height="19" viewBox="0 0 11 19" fill="none" xmlns="http://www.w3.org/2000/svg" className="flex-shrink-0">
