...
export function VacancyStatusCard({
  ...
  const startDate = createdAt ? new Date(createdAt).toLocaleDateString('es-AR') : '—';

  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col gap-4">
      <Typography variant="h3" weight="semibold" className="text-[#737373]">
        {t('admin.vacancyDetail.statusCard.title')}
      </Typography>
      <div className="flex flex-col gap-3">
        ...
        <div className="flex justify-between">
          <Typography variant="body" className="text-[#737373]">{t('admin.vacancyDetail.statusCard.country')}</Typography>
          <Typography variant="body" weight="medium">{country ?? '—'}</Typography>
        </div>
        <div className="flex justify-between">
          <Typography variant="body" className="text-[#737373]">{t('admin.vacancyDetail.statusCard.searchStart')}</Typography>
          <Typography variant="body" weight="medium">{startDate}</Typography>
        </div>
        <div className="flex justify-between">
          <Typography variant="body" className="text-[#737373]">{t('admin.vacancyDetail.statusCard.providersNeeded')}</Typography>
