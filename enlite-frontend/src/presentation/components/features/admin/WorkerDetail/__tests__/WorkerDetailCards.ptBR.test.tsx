...
describe('WorkerStatusCard — pt-BR labels', () => {
  it('renders card title "Status do Worker"', () => {
    render(<WorkerStatusCard {...statusProps} />);
    expect(screen.getByText('Status do Worker')).toBeInTheDocument();
  });
...
describe('WorkerProfessionalCard — pt-BR labels', () => {
  ...
  it('renders knowledgeLevel label "Nível de conhecimento"', () => {
    render(<WorkerProfessionalCard {...professionalProps} />);
    expect(screen.getByText('Nível de conhecimento')).toBeInTheDocument();
  });
